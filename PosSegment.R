PosSegment <- function(TweeSeizoenen){
...
p <- ggplot(TweeSeizoenen) + geom_segment( aes(x=1, xend=2, y=`Position 18/19`, yend=`Position 19/20`, size=.05), color = TweeSeizoenen$Color2, show.legend=F) + 
  ...
  geom_text(label=paste("After Match Day",SpeelRondeEre), x=1.5, y=2, vjust=8, size=4, color = "black") +
  geom_text(label="18/19", x=1, y=2, vjust=8, size=4, color = "black") +
  geom_text(label="19/20", x=2, y=2, vjust=8, size=4, color = "black") + annotate(geom="text", x=2, y=19, label="Made by @RobinWilhelmus in RStudio",
                                                                 color="black", size = 3)
